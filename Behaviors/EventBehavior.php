...
EOF;
        ...
        $script = preg_replace(
            '/(([\t ]+)public function delete.*?try[^{]{)/s',
            '\1'."\n".'\2\2\2EventDispatcherProxy::trigger(array(\'delete.pre\',\'model.delete.pre\'), new ModelEvent(\$this, \$con));',
            $script,
            1
        );
        ...
        $script = preg_replace_callback(
            '/([\t ]+)public function save.*?.*?} catch \(Exception \$e\)/s',
            array($this, 'modelFilterCallback'),
            $script
        );
